...
(defn create-http-rpc-sender
  ...
  (fn [& args]
    (let [[target-node message callback]
      ...
      (go
        (try
          (log/debug "Sending RPC to" target-node "- message:" message "type:" (type message))
          (when (vector? message)
            (log/debug "Message vector - first:" (first message) "type:" (type (first message))))
          ...
          (catch Exception e
            (log/error "RPC error sending to" target-node ":" (.getMessage e))
...
(defn handle-rpc-request
  ...
  (try
    (let [body (-> request :body slurp .getBytes nippy/thaw)
      ...
      (log/info "RPC received - raw body:" body)
      (log/info "Message type:" (type message) "- content:" message)
      (if raft-instance
        (let [result-promise (promise)
              [raw-op data] (if (vector? message) 
                              [(first message) (second message)]
                              [message nil])
              _ (log/info "Extracted raw-op:" raw-op "type:" (type raw-op))
              op (cond
                   (keyword? raw-op) raw-op
                   (string? raw-op) (keyword raw-op)
                   (symbol? raw-op) (keyword (name raw-op))
                   :else (do (log/error "Unknown op type:" (type raw-op) "value:" raw-op)
                            (keyword (str raw-op))))]
          (log/info "Final op:" op "type:" (type op) "- will invoke RPC")
          (try
            (let [event-chan (raft/event-chan raft-instance)]
              (raft/invoke-rpc* event-chan op data
                                (fn [result] 
                                  (log/info "RPC result for" op "-" result)
                                  (deliver result-promise result))))
            (catch Exception e
              (log/error "Failed to invoke RPC - op:" op "error:" e)
    ...
    (catch Exception e
      (log/error "Error handling RPC request:" (.getMessage e) "- exception:" e)
      {:status 500
       :headers {"Content-Type" "application/octet-stream"}
...
(defn handle-command-request
  ...
  [request]
  (log/info "COMMAND: Received command request")
  (try
    (let [body (-> request :body slurp .getBytes nippy/thaw)
          {:keys [raft-instance node-id]} @node-state]
      (log/info (str "COMMAND: " node-id " - Command body: " body))
      (if raft-instance
        ...
        (let [state-promise (promise)
          ...
          (log/info (str "COMMAND: " node-id " - Current status: " (:status current-state) 
                     ", leader: " (:leader current-state)))
          (if (= (:status current-state) :leader)
            ;; We are the leader, process the command
            (let [result-promise (promise)
                  timeout-ms 5000]
              (log/info (str "COMMAND: " node-id " - We are leader, submitting entry to Raft"))
              (raft/new-entry raft-instance body
                              (fn [result] 
                                (log/info (str "COMMAND: " node-id " - Raft callback with result: " result))
                                (deliver result-promise result))
                              timeout-ms)
              (let [result (deref result-promise (+ timeout-ms 1000) {:type :info :error :timeout})]
                (log/info (str "COMMAND: " node-id " - Final result: " result))
                {:status 200
                 :headers {"Content-Type" "application/octet-stream"}
                 :body (nippy/freeze result)}))
            ...
            (if-let [leader (:leader current-state)]
              (do
                (log/info (str "COMMAND: " node-id " - Forwarding to leader: " leader))
                (let [result (forward-command-to-leader leader body)]
                  {:status 200
                   :headers {"Content-Type" "application/octet-stream"}
                   :body (nippy/freeze result)}))
              (do
                (log/info (str "COMMAND: " node-id " - No leader elected yet"))
                {:status 503
                 :headers {"Content-Type" "application/octet-stream"}
                 :body (nippy/freeze {:type :fail :error :no-leader})}))))
        (do
          (log/info (str "COMMAND: " node-id " - No raft instance available"))
          {:status 503
           :headers {"Content-Type" "application/octet-stream"}
           :body (nippy/freeze {:type :fail :error :node-not-ready})})))
    (catch Exception e
      (log/error "COMMAND: Error handling command request:" (.getMessage e) e)
      {:status 500
       :headers {"Content-Type" "application/octet-stream"}
...
(defn start-distributed-node
  ...
  (log/info "Starting distributed Raft node" node-id "on" node-host ":" node-port)
  (log/info "STARTUP TEST: This should appear in logs")
  ...
  (let [cluster-members (parse-cluster-members cluster-members-str)
        ...
        state-machine-fn (fn [entry raft-state]
                          (try
                            (cond
                              ...
                              (do
                                (log/info "State machine received nil/empty entry")
                                (util/ok-result))
                              
                              ;; Check if this is an internal Raft operation (no :f field)
                              (nil? (:f entry))
                              (do
                                (log/debug "State machine received non-application entry:" entry)
                                (util/ok-result))
                            ...
                            (catch Exception e
                              (log/error "State machine error processing entry:" entry "Error:" e)
        ...
        raft-config (util/default-raft-config
                     ...
                     :leader-change-fn (fn [event]
                                         (log/info "LEADER CHANGE -" node-id "- new leader:" (:new-leader event) "event:" event)))
        
        ;; Add snapshot functions (test stubs)
        full-raft-config (merge raft-config
                                {:snapshot-write      (fn [index callback]
                                                        (log/debug node-id "Writing snapshot at index" index)
    ...
    (log/info "Distributed node" node-id "started successfully")
    (log/info "Cluster members:" cluster-members)
    
    ;; Keep the main thread alive with periodic heartbeat
    (loop []
      (Thread/sleep 10000)
      (log/debug node-id "heartbeat - node running")
      (recur))))

(defn shutdown-node
  "Gracefully shuts down the node"
  []
  (log/info "Shutting down distributed node...")
...
(defn -main
  ...
  [& args]
  (when (< (count args) 4)
    (log/error "Usage: distributed-test-main NODE_ID NODE_HOST NODE_PORT CLUSTER_MEMBERS")
    (log/error "Example: distributed-test-main n1 localhost 7001 'n1:localhost:7001,n2:localhost:7002,n3:localhost:7003'")
    (System/exit 1))
  (let [[node-id node-host node-port-str cluster-members-str] args
    ...
    (try
      ...
      (catch Exception e
        (log/error "Failed to start distributed node:" (.getMessage e))
        (shutdown-node)
        (System/exit 1)))))